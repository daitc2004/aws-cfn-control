...
function build_nsd_file {
  ...
  if [[ "$my_instance_type" = "i3.16xlarge" ]]; then
    let tot_i_store_disks=8
  elif [[ "$my_instance_type" = "i3.8xlarge" ]]; then
    let tot_i_store_disks=4
  fi
  let nsd_num=0
  fg=100
  ...
  for s in $servers; do
    ...
    if [[ "$vol_type" = "InstanceStore" ]]; then
      while [[ "$disk_num" -lt "$tot_i_store_disks" ]]; do
